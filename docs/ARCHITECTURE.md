...
> **Aanvulling 2026-08-05: welke ref een tenant leest, hangt af van `tenant.wave` en
> `tenant.environment`.** De ApplicationSet zet `targetRevision` per tenant:
> `wave: "0"` volgt `HEAD` (main, 2 tenants), `environment: accept` volgt
> `release-accept` (48), de rest volgt `release` (26). De keten is dus
> main → `release-accept` → `release`, met een gezondheidscheck tussen elke stap.
> (Niet `tenant.canary` — die vlag hoort bij de geparkeerde emptyDir/S3-PoC in
> `canary-overrides.yaml` en staat op geen enkele tenant.) Een merge naar main raakt dus
