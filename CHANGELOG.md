...
Geen bestaande tenant raakt hierdoor van engine: alle 77 bestanden in
`values/tenants/` hebben al een expliciete `tenant.dbType` (55 postgres, 22
mariadb), en `.tenant.dbType` stond al in `REQUIRED_FIELDS` van
`scripts/validate-values.sh`. De default is dus een vangnet, geen werkende
